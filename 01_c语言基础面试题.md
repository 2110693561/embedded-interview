...
> 来源: https://wdxzf.github.io/interview/%E5%B5%8C%E5%85%A5%E5%BC%8F%E5%85%AB%E8%82%A1%E6%96%87/01_c%E8%AF%AD%E8%A8%80%E5%9F%BA%E7%A1%80%E9%9D%A2%E8%AF%95%E9%A2%98
...
**常见面试题：** `extern` 和 `static` 能不能同时用？ → `static` 限定为本文件，`extern` 声明外部文件，语义冲突。
...
> 🧠 **秒懂：** const int *p（指向的值不能改），int _const p（指针本身不能改），const int _const p（都不能改）。记忆口诀：const在_ 左修饰值，在_右修饰指针。
...
指针数组(int _arr[5])：5 个元素的数组，每个元素是 int_ 。数组指针(int (_p)[5])：一个指针，指向含 5 个 int 的数组。区别：sizeof(arr)=5_ 8=40字节(64位), sizeof(p)=8字节。二维数组传参时用 int (*p)[cols] 形式保留列信息。函数指针数组：int (*fptrs[3])(int) 是 3 个函数指针的数组。
...
> 💡 **面试追问：** “const int _p 和 int _const p 区别？” → 前者指向的值不能改(底层const),后者指针本身不能改(顶层const)。记忆: const在_ 左边修饰指向物,在_右边修饰指针本身。

* * *

### Q128: 实现memcpy？

> 🧠 **秒懂：** 逐字节拷贝src到dst：while(n—) _d++ =_s++。注意：不处理内存重叠，重叠时应该用memmove。void_ 参数需强转为char_才能逐字节操作。
...
> 🧠 **秒懂：** const在 _左(指向const数据)、const在_ 右(const指针)、双const(都不可变)。再加上无const(都可变)共四种组合。
...
> 🧠 **秒懂：** 将整数地址强转为函数指针并调用：void (_fp)(void) = (void(_)(void))0x08000000; fp();。嵌入式中用于跳转到Bootloader或应用程序入口。
